...
LastEditTime: 2022-12-27 17:59:15
...
def _uint32_swar(x: np.uint32) -> np.uint32:
...
class LFSR:
    ...
    def get(self):
        ...
        out_bit = self._reg & 0x1
        fb_status = self._reg & self._g
        fb_val = 0
        while fb_status > 0:
            fb_val += _uint32_swar(np.uint32(fb_status & 0xffffffff))
            fb_status >>= 32
        self._reg >>= 1
        self._reg |= (fb_val & 0x1) << (self._l - 1)
        return out_bit
        
        
if __name__ == "__main__":
    lfsr = LFSR(10, 0b1011100001, 0b0101010101)
    for i in range(30):
        print(lfsr.get(), end='')
