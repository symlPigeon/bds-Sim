'''
Author: symlpigeon
Date: 2022-11-08 12:32:11
LastEditTime: 2022-11-08 15:22:54
LastEditors: symlpigeon
Description: CRC-24Q编码
FilePath: /sim_bds/python/coding/crc24q.py
'''
...
def crc24q_gen(data: bytes) -> bytes:
    crc = 0
    for i in range(len(data)):
        crc = (crc << 8) ^ crc24_table[(data[i] ^ (crc >> 16)) & 0xff]
    crc &= 0xFFFFFF
    return crc.to_bytes(3, 'big')


def crc24q_check(data: bytes) -> bool:
    crc = 0
    for i in range(len(data)):
        crc = (crc << 8) ^ crc24_table[(data[i] ^ (crc >> 16)) & 0xff]
    crc &= 0xFFFFFF
...
if __name__ == "__main__":
    ...
    # checksum should be b"8!~"
    crc_message = message + crc24q_gen(message)
    print(crc24q_check(crc_message))
    print(crc_message)
