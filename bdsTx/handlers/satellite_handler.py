...
LastEditTime: 2022-11-22 19:50:55
...
class satelliteHandler(baseSourceHandler):
    def __init__(self, prn: int, manager: satelliteQueueHandler, next_handler: baseSourceHandler):
...
class b1cSatelliteHandler(satelliteHandler):
    def __init__(self, prn: int, manager: satelliteQueueHandler, next_handler: baseSourceHandler, frame_order: List[int]):
        super().__init__(prn, manager, next_handler)
        self._iono_corr = manager.get_iono_corr("bdgim")
        self._frame_order = frame_order
        self._frame_creater = b1c_frame.b1cFrame(prn, self._ephemeris, self._iono_corr["alpha"], self._frame_order, self._ldpc_mat_1, self._ldpc_mat_2)
